...
%If a digital certificate cannot be obtained, either because there are problems with conformance to standards and/or security properties, then the code will be fed back to the \emph{self-healing phase} until it does.
\end{itemize}
...
%  \vspace{-3cm}
  \caption{Executive dashboard of the YAG-Suite (French version)}
...
\paragraph{Digital banking}
\label{sec:banking}
As digital disruption moves banking services closer to the end user, more diverse software tools are deployed into the banking workflow via digital banking solutions. We plan to look at how we auto-generate data processing code for a processing hub for the Competition and Markets Authority Nine (CMA9) that forms the nine largest banks in the UK. We will use the tools we develop within \TheProject{} to verify that the auto-generated code is secure and without unwanted vulnerabilities. 
...
%- As more sensors enter into the Distributed healthcare (IoT) domain the code for these sensors, edge collection devices and core processing capability will becoming a life-threatening issue if any vulnerabilities cause unwanted outcomes for a patient's health. We want to ensure auto-healing of our solution and indirectly auto-healing our citizens' healthcare ecosystem.
% what does the distributed healthcare mean though? THE, you obtain distribution because of the devices, not
% because of something else...
% he says distributed here, but global in WP7 ... random - distributed kind of makes sense
% also open vs digital banking
% I think we pick one for each and stick to it - what do you mean?
% well we say open in one place and digital in the other, distributed in one and global in the other. Does it matter? what do we say in WP7?digital banking and global healthcare - no, only healthcare now <- yes you are right
% so do you think we should change it here? but you are right, it should be digital banking. I have changed it here OK -- look - he writes to secure the code is secure !!!! ??
As more medical devices are used in remote healthcare and connected through the Internet, the code for these medical devices, including associated data collection and core processing capabilities, can be susceptible to life-threatening attacks 
%issue 
if any security vulnerability can be breached. %, causing unwanted outcomes to a patient's health. 
%We want to ensure auto-healing of our solution and indirectly auto-healing our citizens' healthcare ecosystem.
We want to increase the trust in the widespread use of such devices and remote consultations, and hence
to ensure that they are safe and secure, and have been rigorously scrutinised for guaranteeing that they preserve the privacy for the patients at all times.
...
improve the quality of software development, data analytics and services to a level that not only has them pass regulatory inspection, but rises to the challenge faced by operating live services in an environment that no longer relies purely on closed networks to achieve secure operation. This ties in with industrialisation efforts currently under way. It is envisioned that lessons learned and \TheProject{} tools will directly flow into the MosaiX data platform, ensuring that our customers can benefit from the use of these technologies in a world where attacks are going to be far more sophisticated and wide-ranging than what is current. 
\end{mdframed}
...
\begin{mdframed}[backgroundcolor=gray!10]
\paragraph{How Will \TheProject{} Achieve This Impact:}
Self-recovery is one of the main themes of the \TheProject{} project, addressed by our self-healing technology based on formally verified code refactoring and runtime adaptation. We will, furthermore, integrate this technology into the overall software engineering methodology for developing secure and privacy-preserving AI-based big data analytic applications, making it transparent to the end user and therefore significantly easing the development of such applications. 
\end{mdframed}
...
\begin{mdframed}[backgroundcolor=gray!10]
\paragraph{How Will \TheProject{} Achieve This Impact:}
\TheProject{} will develop machine learning based runtime monitoring mechanisms that will continually monitor the application execution for security threats. Together with the mechanisms for runtime adaptation that will ensure, through a number of steps, that the overall system is in a stable and secure state, this will ensure that we cannot only identify the known vulnerabilities, but also, by continuously updating the learning model, identify any new threats as they arise. \TheProject{} will also build on  existing security standards (e.g. for C++) and will continually extend them as new types of threats and vulnerabilities are identified. This will ensure that new vulnerabilities and threats are addressed in the upcoming coding standards. 
...
\begin{mdframed}[backgroundcolor=gray!10]
\paragraph{How Will \TheProject{} Achieve This Impact:} 
\TheProject{} will build source code analytic tooling to ensure the conformance of
%, for instance, C++ SIMPLER :D OK?
code (e.g. for C++) with state-of-the-art coding standards. We will also extend these standards  by providing new rules and guidelines specifically aimed at composing the large-scale distributed applications from multiple layers of libraries. This will contribute to better standardisation of the network systems. Furthermore, the formal methods for proving the security and privacy of systems will provide an additional formal assurance for the security of software, further allowing better-informed investment decisions with respect to security and privacy.
...
\paragraph{How Will \TheProject{} Achieve This Impact:} Several technologies of \TheProject{} (e.g.~identity management, the intelligent user interface, and so on) will be designed and developed following a user-centred design approach with the active participation of software developers and end users, in such a way that evaluation feedback gathered through user studies will lead the development process. 
...
Since the results target real-world use and aspire to ultimately improve work processes of certain industries, such a user-centred design approach for evaluating the various technologies %\emph{in their use} 
is considered essential. In addition, the proposed user-centred design approach will focus on improving the usability and user experience of security mechanisms, which nowadays are key aspects for  end user interactive systems, and essential to enhance user acceptance and trust towards the developed technologies.
...
\begin{mdframed}[backgroundcolor=gray!10]
\paragraph{How Will \TheProject{} Achieve This Impact:}
The main target of the \TheProject{} technologies are AI-based products and specifically the emerging large-scale AI-based big data analytics. Our focus is on multi-layered big data analytics applications that are deployed on distributed infrastructure, and combine generic big data analytics processing platforms such as Hadoop with higher-level machine learning frameworks such as Azure AI. This means that our technologies are applicable to the widest range of AI-based products and systems. This will be demonstrated in a variety of domains including those from the  use cases provided by \SOPRAshort{} and \FRQshort{}. 
...
\begin{mdframed}[backgroundcolor=gray!10]
\paragraph{How Will \TheProject{} Achieve This Impact:}
Industry partners involved in the \TheProject{} project, specifically \IBMshort{}, \YAGshort{} and \COGNIshort{}, aim to deliver technologies that will be of a TRL 6 and 7, which indicates technologies or system prototypes demonstrated in an industry relevant environment (e.g. the \SOPRAshort{} and \FRQshort{} use cases). This means that \TheProject{} will produce technologies that will be closer to being usable in practice. The technologies developed will be either open source, where feasible, or available as parts of commercial tool suites. This, coupled with the educational program using MOOCs to promote \TheProject{} technologies, will contribute towards a more competitive EU cybersecurity industry reducing dependence on importing technologies from abroad.
...
\noindent
\TheProject{} will significantly improve long-term innovation capacity by enabling European companies and individuals to develop complex and innovative new secure large-scale big data analytics applications for a variety of commercially-important and emerging markets, including healthcare, air traffic management and digital banking. A combination of different methods for identifying and repairing security risks and privacy leaks, coupled with formal methods for proving security properties, and continuously evolving security standards, will allow the identification and repair of not only current, but also future security threats, making software developed using our technologies robust and resilient in the future, too. The \TheProject{} methodology, together with the associated tool chain, will significantly reduce the level of expertise required from developers to write secure code, allowing non-experts in this area to develop software that is provably secure, and automatically checked for conformance to standards. This will bring a major boost to the European software market, as security and privacy will become essential when designing and implementing new software, especially in the light of the increased distribution of data across domains in parallel with more and more strict regulations governing privacy and data ownership. SMEs involved in the project will use our software solutions to identify additional market opportunities, and as such \TheProject{} will support the growth of these companies by significantly extending the services they are able to offer. %where? Page 27, Expected Impact 2. OK lets go back
Section~\ref{sec:innovationpotential} (Page \pageref{sec:innovationpotential}) describes the market opportunities and potential for innovation in more detail.

\paragraph*{Societal Impact.}
\noindent
\TheProject{} will have a major impact on society through its focus on delivering secure and privacy preserving data analytics code. Our methodology will allow for \emph{secure-by-design} application development, meeting the ever-growing societal needs for secure complex data analysis whilst ensuring data privacy in light of new and stricter regulations such as GDPR. One of our goals is to support companies in enforcing compliance to GDPR in their applications by developing \emph{privacy-by-design} technologies and a supporting methodology. With use cases from various domains, we will also deliver significant impact to areas not directly related to computer science
but increasingly reliant on data intensive systems and data analytics. By investigating mechanisms for secure knowledge transfer between geographical areas, we will enable faster and  secure cooperation between institutions, which is especially important in the healthcare domain, where there is a growing trend towards decentralisation of the data and diagnostics, and for shared learning that breaks the boundaries between institutions and countries. Remote collaboration and knowledge sharing is of critical importance to the world affected by the current pandemic,  where knowledge exchange will need to be facilitated in the presence of strict social distancing rules.

\begin{mdframed}[backgroundcolor=blue!5]
\label{box:gender}
\textbf{Gender Imbalance.} Gender imbalance in Computer Science is a particularly important area for societal impact that the \TheProject{} project is committed to reducing. \TheProject{}  will address it in several ways. We will use dissemination and engagement activities with the general public to inspire and educate future generations of software developers and end users to the importance of security and privacy in big data analytics, making a particular effort to engage with mixed audiences. Our Project Coordinator is particularly enthusiastic about talking to the general public and has experience in doing so (e.g. events organised by local communities, Fringe and science festivals in Edinburgh, etc). % science festival in Edinburgh? that would be cool too
We will stress the importance of data protection and privacy-preserving mechanisms in AI solutions, and therefore in protecting personal data (such as gender, ethnicity, etc.).
%\TheProject{} is concerned with the protection of personal data (such as gender, ethnicity, etc.). %% <--- this sentence comes out of the blue - this was the old sentence
The framework for examining the right to privacy from a gender perspective is found in the Universal Declaration of Human Rights (Article 12), the International Covenant on Civil and Political Rights (Article 17) the Resolutions of the United Nations' General Assembly and Human Rights Council, and the international human rights legal framework. The UN Human Rights Council (HRC Resolution 34/7) has noted that \textbf{``violations and abuses of the right to privacy in the digital age may affect all individuals, including with particular effects on women, as well as children and persons in vulnerable situations, or marginalized groups''}. 
%By researching improved privacy-preserving mechanisms, 
\TheProject{} makes a direct contribution to preventing violations and abuse of the right to privacy in the digital age with focused research in privacy-preserving algorithms.
In addition, by explicitly looking at the privacy-preserving methods for training machine learning models, we will investigate how 
%are hoping 
to reduce 
the well known bias in training sets for ML (which is currently towards white men).
%\vjcomment{Evidence of this?}. 
%By explicitly addressing privacy-preserving issues %of preserving the privacy of data, 
%we are hoping to help reduce gender bias and under-representation of minorities. Indeed, Data Science is one of the areas of Computer Science which has much more gender balance than the others. 
%still confused - what are we trying to say?
% there seems to be three different things here 1. addressing gender imbalance (although we don't really say how) 2. privacy of data (I don't understand what this has to do with gender imbalance) 3. education which improves balance
% OK
%I guess we need to say that we will use education and role model as a way of tackling imbalance in CS
% the point about privacy of data is about protection the individual and hence minorities. basically making it easier for minorities to express an opinion since they know their confidentiality will not be breached... something like that i think
% perfect, then I suggest we order it like that and basically just ... say that.. I think this is over complicating the issues in places, too :D OK. I will have a shot and you can correct me cool
% difficult...
Finally, by providing an educational program that will bring together Data Science with areas that are traditionally dominated by men (such as Cybersecurity), we are hopping to contribute to reduce the imbalance in the latter. 
%Finally,  \vjcomment{How?} \vjcomment{We also need a reference to the Stanford paper about equality.}
\end{mdframed}
 %I think the last sentence is still out of order
...
expertise in big data analytics (\UODshort{} and \SOPRAshort{}). Adding to this the industrial use cases that will be considered (\SOPRAshort{} and \FRQshort{}), as well as the expertise in conformance to standards, refactoring, formal specification and verification of properties (\SAshort{} and \UCMshort{}), our consortium is ideally placed to tackle the problem presented. Therefore, the quantifiable benefits should be fully attainable.
\\ \hline
\addtocounter{barrier}{1}
\noindent
\emph{Barrier \thebarrier.}
...
&
\noindent
The benefits of the \TheProject{} methodology will be explained clearly and supported by both mathematical and software engineering foundations, through formal reasoning and design patterns. Evidence of the improvements in security and privacy will be communicated to possible users and the \TheProject{} technologies and supporting methodology will be promoted to wide audiences by MOOCs. Furthermore, the project consortium contains experienced software
...
We will do this through a mixture of high-quality publications, presentations
...
developer and user communities through a series of focused activities, the most important of which will be organisation of massive online courses via MOOCs, allowing us to engage with a huge base developers and to promote the \TheProject{} technologies.
...
\COGNIshort{} will broaden its expertise and extend its products and services in the field of usable security, intelligent user interfaces and artificial intelligence aiming to help businesses in Europe to design, develop and integrate human-centred cybersecurity systems.
%
...
\begin{itemize}
    ...
    and services with benefits provided by Blockchain technology such as decentralised and security features.
...
Ville Voutilainen (Senior Engineer Qt Company, head of delegation of the Finnish National Body for C++ standardisation, former chair of the Evolution Working Group at the ISO/IEC JTC1/SC22/WG21), and 
Ren\'e Mayrhofer (Head of Networks and Security at JKU, Consultant for Android Security, Google).

%He is also the head of delegation of the Finnish National Body for C++ standardisation.
