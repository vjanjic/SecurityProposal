...
could provide strong end-to-end security guarantees throughout system execution and across layers of abstraction. They have indeed had significant success over the years  in tackling security related problems, such as reasoning about the correctness of cryptographic algorithms~\cite{FM-crypto} or specifying and enforcing privacy guarantees~\cite{FM-priv-guarantee}. 
...
\begin{mdframed}[backgroundcolor=blue!5]
\paragraph{End-Users and Software Developers.}
Most of the technologies that will be developed over the course of the \TheProject{} project are aimed at assisting \emph{software developers} in developing and deploying their code. This includes the \TheProject{} methodology, as well as the tools for detecting and repairing security vulnerabilities, verification of the properties of the code, code analysis for conformance to coding standards and authentication mechanisms. All this will increase the trust in the developed big data analytics systems. However, \TheProject{} will also benefit the \emph{end users} of the big data applications. This is most notable in the considered use cases, that will directly benefit the end-users by XX. Additionally, the authentication mechanisms will allow these users to XX. 
\end{mdframed}
...
In the \TheProject{} project, we will develop an Identity-as-a-Service (IDaaS) technology that will allow the developers to easily integrate, deploy and manage methods for identity, authentication and access management depending on customer requirements and policies. In particular, we will develop and evaluate state-of-the-art multi-factor authentication, and intelligent biometrics based on a combined analysis of physiological, facial and eye gaze data analytics for continuously identifying the end-users. In addition, we will develop a blockchain solution for user access control, as well as data analytics and reporting services aiming to provide intelligent insights from users' biometric data allowing developers to deliver personalised security experience and thus increase user acceptance and trust. Finally, from the end-users’ point of view, the developed IDaaS technology will provide a fluid user experience based on state-of-the-art authentication and access control methods such as easy-to-use tokens, usable, single-touch user approvals, as well as intelligent biometrics based on users' physiological, facial and eye gaze data analytics.
