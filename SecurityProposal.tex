...
techniques in terms of instances of parallel patterns. The examples of these libraries are Keras(\url{\url{keras.io}}) and Spark MLLib. These libraries abstract over the data processing and data storage frameworks, combining them %together 
...
& ALL & 
It is possible that external events, such as administrative problems due to Brexit or required social distancing due to the ongoing COVID-19 pandemic, can impact the communication and collaboration between the project partners. \TheProject{} will take all measures to minimise the impact of Brexit, by using computational clouds (with the actual machines situated in Europe) for processing and storage of the project data, therefore avoiding potential administrative problems with data ownership and sharing. The project partners are well used to online collaboration, being members of various EU project consortiums that have switched to online meetings during the ongoing pandemic, and are thus able to continue to do so
%the continuation of such %social distancing 
%measures will not 
without impacting the progress of the project.
