...
%We will produce a methodology for developing secure distributed data analytics applications, that will be based on on formally specified, machine readable and verifiable \emph{security contracts} for specifying security aspects of the sensitive parts of the distributed code.
...
{\color{blue} \SOPRAshort{} update CI/CD proposal}
As the solutions in the distributed data processing ecosystem is an amalgamation of a diverse assembly of various components. As Continuous Integration (CI) and Continuous Deployment (CD) enables the development of new or changes code for the system to enable continuous improvement of the functionality of the solution. To ensure success in the deployment of each integration requires verification by an automated build and automated tests suite. The security assessment build by our research will act as a critical test before deployment and integration. The reason been that the introduction in the processing pipeline could cause security vulnerabilities in other parts of the pipeline. The CI/CD process will ensure this vulnerabilities do not enter the live ecosystem because of the testing rules introduce new functionality. During the automatic run-time monitor process the CI/CD enables an auto-healing of the code base.

\begin{figure}[tp]
  \begin{center}
  \vspace{-5mm}
  \includeimage[scale=0.4]{Digital-Fortress-CD-CI.jpg}
%  \vspace{-3cm}
  \caption{Continuous Integration (CI) / Continuous Deployment (CD) for \TheProject{} Methodology}
  \label{fig:ci:cd}
  \end{center}
  \end{figure}
...
\item \textbf{Vulnerability detection and self-healing.}
The project will investigate three mutually dependent challenges: (1) the formulation of constraints (contracts) that should be satisfied (2) the specification of detection algorithms that uncover any constraint violation, and (3) the specification of repair and adaptation algorithms that restore the constraints (self-healing). The capture of security requirements in distributed AI-based data analytics systems is still neglected, but in general, security in computing systems is  well-studied. The common state-of-the-art approach is to start from an attacker model and system vulnerabilities. With respect to possible attacks the project will focus on unauthorised intrusion with the potential of causing (planned or carelessly) damage to the system, i.e. identity management and access control are primary objects of interest. For this the project will take an approach based on anomaly detection and counter-measures based on the adaptation of the security controllers. The anomaly detection will be based on the learning of the protocol language used in the interaction with the system, and the counter-measures will change the rules of the identity manager and the access control. Then the verification of security requires proofs that an anomaly, once it has been detected, will be removed by the counter-measures after a pre-determined maximum number of steps. The challenge is to investigate such novel class of self-healing proof obligations concerning security requirements, which goes beyond the current capabilities of the rigorous methods and their supporting tools. Furthermore, the research on anomaly detection will not only focus on the recognition of anomalies that may indicate unauthorised and potentially malicious intrusion and the adequate countermeasures that repair a violation, but also on the early anticipation of damaging activities and the start of counter-measures before any damage has occurred. For instance, components may produce incorrect Byzantine behaviour or not react anymore, or components may simply be jammed. In order to cope with such vulnerabilities, the challenge is to identify also states that are close to states that will require counter-measures and sequences of update sets by individual agents that led to these states, such that the combination of both indicate the potential creation of a non-acceptable state within a small number of steps. 
...
\begin{figure}[tp]
  \begin{center}
  \vspace{-5mm}
  \includeimage[scale=0.6]{DigitalFortress.pdf}
%  \vspace{-3cm}
...
{\color{blue} \YAGshort{ question} \textbf{Is this section referring only to research labs projects, ie what about industry research and innovation ?}} 
...
\cbcomment{I think this is too detailed and technical for this section.}
...
YAGAAN developed source code vulnerability scanning technologies based on static analysis and machine learning, with a strong focus to remediate the state of the art limitations of Static Analysis Security Testing such as false positives or duplicate warnings. YAGAAN developed the YAG-Suite product which is a suite of tools that use static analysis to scan source code and raise a warning when a suspicious sequence of code is detected as well as machine learning to automate warnings qualification with supervised training and provide decision making information to the user such as vulnerability diagnostics and remediation options, Machine learning is also used to handle uncertain information, contextualize the analysis and estimate relevancy (likeliness of being a true positive) and criticality (CVSS score
\footnote{CVSS: Common Vulnerability Scoring System v3.1 Specification Document (\url{https://www.first.org/cvss/})}
) metrics for each individual warnings.
In order to detect sensitive data leaks, and in particular privacy violation risks, the YAG-Suite also integrates business oriented semantics to detect vulnerabilities.
YAGAAN also developed code mining technology to support deep and expert investigation of applications, allowing the user to query specific properties  of interest and analyse corresponding areas of the source code, be they certain like database queries for instance or uncertain (known as "code smells"
\footnote{Code smell: symptoms of poor design and implementation choices - M. Fowler, K. Beck, J. Brant, W. Opdyke, and D. Roberts, Refactoring: Improving the Design of Existing Code. Addison-Wesley, 1999.}
).
...
\item availability of advanced tools and services to the CERTs/CSIRTs and networks of CERTs/CSIRTs;
\item an EU industry better prepared for the threats to IoT, ICS (Industrial Control Systems), AI and other systems;
\item self–recovering, inter-operable, scalable, dynamic privacy-respecting identity management schemes.
\end{itemize}
...
\begin{wrapfigure}{R}{2cm}
\vspace{-1.4cm}
\hfill \includeimage{logos/yagaan.jpg}
\vspace{-0.9cm}
\end{wrapfigure}
