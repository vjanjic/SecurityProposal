...
\begin{itemize}
\item \emph{Vulnerabilities detection phase}, where the tool-chain will identify potential vulnerabilities in the code, both in terms of potential targets for cyber attacks (security) and in terms of potential leakage of information (privacy). The identification will combine static code analysis that will be developed by \YAGshort{} and \UCMshort{} with dynamic symbolic code execution mechanisms that will be developed by \IBMshort{}. Combining these two complementary methods will allow us to discover a wide range of potential vulnerabilities in the code, as well as to avoid false positives, that are common problem if only static code analysis is used. The general techniques for detecting vulnerabilities will be adapted to the specific problems of data storage and processing in distributed data analytics by \UODshort{} and \SCCHshort{}. 

\item \emph{Self-healing phase}, where, after the potential vulnerabilities have been identified, the code will be transformed using semi-automatic code refactoring techniques that will be developed by \SAshort{}, which themselves will be driven by machine-learning techniques to identify what code repairing methods are needed. These methods will be developed by \DEMshort{} and \SCCHshort{}. The output of this phase is the repaired code, where the identified vulnerabilities have been patched.

\item \emph{Formal verification of code properties}, where the resulting code conforms to the required security standards and that it satisfies the required security properties. The conformance to coding standards will be based on the static analysis of the code (using techniques that will be developed by \UCMshort{}), while the security properties will be verified using formal methods that will be developed by \SAshort{} and \YAGshort{}. The output of this phase will be the final secure code, together with some form of a \emph{digital certificate} that will specify what standards the code conforms to and what security properties does it satisfy. Depending on the results of this phase, the code might need to be fed back to the \emph{self-healing phase}, if further problems with conformance to standards and/or security properties have been identified.
\end{itemize}
