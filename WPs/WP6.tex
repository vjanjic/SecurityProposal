...
In \theTask we will produce a new software engineering methodology for the development of secure continuous deployment methodologies.  During continuous deployment, code is typically compiled on the fly on a deployment server before being deployed to a test or run time server.  In this task, we will look to assess the suitability of the tools and techniques for continuous deployment and security weakness in the tooling chain.  We will assess existing  continuous deployment  methods and their applicability to develop secure-aware applications for big data.
\end{Task}
